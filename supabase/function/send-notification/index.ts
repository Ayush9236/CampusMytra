...
async function sendFcm(token: string, accessToken: string, notif: { title: string; text: string }, data: Record<string, string>): Promise<boolean> {
  const res = await fetch(FCM_URL, {
    ...
    body: JSON.stringify({
      message: {
        ...
        android: {
          notification: { channel_id: 'campus_mytra', priority: 'high', color: '#818CF8' },
          priority: 'high',
        },
        data,
      },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const profile of tokens) {
      const ok = await sendFcm(profile.fcm_token, accessToken, notif, dataPayload);
      if (ok) sent++;
    }
